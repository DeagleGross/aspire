...
param inventory_backendDomain string

@description('The name of the inventory-api container app.')
param inventory_backendAppName string = 'inventory-api'
...
resource inventoryBackend 'Microsoft.ApiManagement/service/backends@2024-05-01' = {
  name: 'inventory-backend'
  properties: {
    protocol: 'http'
    url: 'https://${inventory_backendAppName}.${inventory_backendDomain}'
  }
  parent: apim
}

resource inventoryPolicy 'Microsoft.ApiManagement/service/apis/policies@2024-05-01' = {
  name: 'policy'
  properties: {
    format: 'xml'
    value: '<policies><inbound><base /><set-backend-service backend-id="inventory-backend" /></inbound><backend><base /></backend><outbound><base /></outbound><on-error><base /></on-error></policies>'
  }
  parent: inventoryApi
}

resource inventory_catchallOperation 'Microsoft.ApiManagement/service/apis/operations@2024-05-01' = {
  name: 'catchall'
  properties: {
    displayName: 'Catch-all'
    method: 'GET'
